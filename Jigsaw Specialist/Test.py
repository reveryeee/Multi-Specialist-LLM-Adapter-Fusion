...
ADAPTER_PATH = "jigsaw_adapter" 
GOLDEN_JSON = "golden_test_set.json"
...
for item in tqdm(golden_test_set["snli"]):
    res = get_response("Answer with only one word: entailment, neutral, or contradiction.",
                       f"P: {item['premise']}\nH: {item['hypothesis']}")
...
for item in tqdm(golden_test_set["squad"]):
    res = get_response("Answer the question based on context. Shortest answer possible. If no answer, say 'unanswerable'.",
                       f"C: {item['context']}\nQ: {item['question']}")
...
# ==========================================
print("\n" + "="*40)
print("[Jigsaw-Specialist Model Results]")
print(f"SNLI Score:   {results['snli']/SAMPLE_SIZE:.2%}")
print(f"Jigsaw Score: {results['jigsaw']/SAMPLE_SIZE:.2%}")
print(f"SQuAD Score:  {results['squad']/SAMPLE_SIZE:.2%}")
print("="*40)
